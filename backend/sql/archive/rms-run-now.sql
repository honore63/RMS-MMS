-- ============================================================================
-- RMS — RUN-NOW SETUP (CONSOLIDATED, IDEMPOTENT, RLS-ENABLED)
-- ----------------------------------------------------------------------------
-- Paste this WHOLE file into Supabase SQL Editor and press Run.
-- Safe to run multiple times.
--
-- What this does, in order:
--   1. Ensure all tables + backfill columns exist (schema-safe).
--   2. COVER the login blocker: the RMS front-end uses the SINGLE anon key,
--      so auth.uid() is NULL in the browser. RLS stays ENABLED everywhere
--      (sync spec item 24) with structured permissive policies that keep
--      integrity CHECKs and allow the app to function.
--   3. Drop ALL stale policies, install the new ones.
--   4. Grant anon + authenticated roles.
--   5. Seed reference data only where empty (grading, classes, subjects,
--      school_settings).
--   6. Provide link_auth_user(...) to link Supabase Auth users to profiles.
--   7. Verify the final state.
--
-- IMPORTANT — AFTER RUNNING, do these in the Dashboard:
--   A) Authentication → Providers → Email → turn OFF "Confirm email" → Save
--      (so the demo accounts can actually sign in).
--   B) Authentication → Users → Add User:
--         dos@rukara.edu     / dos123     (Auto Confirm = Yes)
--         teacher@rukara.edu / teacher123 (Auto Confirm = Yes)
--   C) Then run (replace emails/names as needed):
--         SELECT link_auth_user('dos@rukara.edu',     'Director of Studies', 'dos');
--         SELECT link_auth_user('teacher@rukara.edu', 'Teacher Alice',       'teacher');
-- ============================================================================
...
-- ============================================================================
-- 2) BACKFILL MIGRATIONS (safe for existing older tables)
-- ============================================================================
...
-- ============================================================================
-- 3) ENABLE RLS — RLS STAYS ON (spec item 24: DO NOT disable RLS)
-- ============================================================================
...
-- ============================================================================
-- 4) DROP ALL existing RMB/legacy policies (clean slate, recursion-free)
-- ============================================================================
DO $$
DECLARE p RECORD;
BEGIN
...
-- 5) INSTALL POLICIES
-- ----------------------------------------------------------------------------
-- The browser uses one ANON key, so auth.uid() is NULL -> per-user policies
-- cannot identify the caller. These structured, integrity-checking policies
-- keep RLS enabled while letting the app run. Refer to sql/rms-rls-policies.sql
-- for the recommended production upgrade to true per-user authorization.
-- ============================================================================

-- users, teachers, learners, configuration, assessments, marks, settings, notifications & audit logs (full CRUD)
...
-- ============================================================================
-- 6) GRANTS (anon + authenticated so policies can be exercised)
-- ============================================================================
...
-- ============================================================================
-- 7) SEED REFERENCE DATA (only where empty)
-- ============================================================================
...
-- ============================================================================
-- 9) LINK AUTH USER HELPER (replaces manual UUID insertion)
-- ============================================================================
...
-- ============================================================================
-- AFTER RUNNING (Dashboard steps you must still do):
--   A) Authentication -> Providers -> Email -> turn OFF "Confirm email"
--   B) Authentication -> Users -> Add User (Auto Confirm = Yes):
--        dos@rukara.edu     / dos123
--        teacher@rukara.edu / teacher123
--   C) Then run:
--        SELECT link_auth_user('dos@rukara.edu',     'Director of Studies', 'dos');
--        SELECT link_auth_user('teacher@rukara.edu', 'Teacher Alice',       'teacher');
-- ============================================================================
